...
function updateScene(event) {
  // Verifica si el clic fue dentro del div principal
  if (mainDiv.contains(event.target)) {
    // Si el clic fue dentro, no hagas nada
    return;
  }

  // Si el clic fue fuera, procede con la actualización de la escena
...
function createScene() {
  // Genera un número aleatorio y decide si será un tono cálido bajo (0-60) o alto (300-360)
  if (Math.random() > 0.5) {
    color = Math.floor(Math.random() * 60); // Tonos rojos a naranjas
  } else {
    color = Math.floor(Math.random() * 60) + 300; // Tonos magentas a rojos cálidos
  }
    ...
    for (var y = -size * 0.5; y < size * 0.5; y++) {
      for (var z = -size * 0.5; z < size * 0.5; z++) {
        ...
        TweenMax.to(cube.scale, 2, {
          ...
          ease: Power2.easeInOut,
          delay:
            -Math.round(noise.simplex3(x * 0.08, y * 0.08, z * 0.08)) - 1.5,
        });
        cubes.add(cube);
      }
    }
